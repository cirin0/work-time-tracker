...
return new class extends Migration {
    ...
    public function down(): void
    {
        if (Schema::getConnection()->getDriverName() === 'pgsql') {
            ...
            DB::statement('ALTER TABLE leave_requests DROP CONSTRAINT IF EXISTS leave_requests_type_check');
            DB::statement("ALTER TABLE leave_requests ADD CONSTRAINT leave_requests_type_check CHECK (type IN ('sick', 'vacation', 'personal'))");

            return;
        }

        Schema::table('leave_requests', function ($table) {
            $table->enum('type', ['sick', 'vacation', 'personal'])->change();
        });
    }
};
